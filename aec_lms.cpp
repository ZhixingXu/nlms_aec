...
#else
        fseek(fp_mic,(1-filter_order)*sizeof(uint16_t),SEEK_CUR);
        fseek(fp_ref,(1-filter_order)*sizeof(uint16_t),SEEK_CUR);

        float out=input.back()-sum(farend*auto_adapt_filter);
        uint16_t dat=big2little(uint16_t(out*65536));
        fwrite(&dat,sizeof(uint16_t),1,fp_aec);
#endif
    }
    fclose(fp_mic );
    fclose(fp_ref );
